...
DROP TABLE IF EXISTS `inconceivable_countries`;
CREATE TABLE `inconceivable_countries` (
...
INSERT INTO `inconceivable_countries` (`id`, `id_parent`, `id_status`, `name`, `date_reg`, `date_upd`, `ip_upd`, `ref`, `loops_ref`) VALUES
(1, 0,  1,  'Spain',  NOW(),  NOW(),  '0.0.0.0', LEFT(UUID(),8), 1);

DELIMITER ;;

CREATE TRIGGER `serialize_country_name` BEFORE INSERT ON `inconceivable_countries` FOR EACH ROW
BEGIN
