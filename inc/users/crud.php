  ...
  function createOne() {
    ...
    if(isset($this->email)) :
      ...
      if(!filter_var($this->email,FILTER_VALIDATE_EMAIL)) :
        ...
        $this->query = "SELECT ".$this->tableletter.".`id` FROM `".$this->table_name."` ".$this->tableletter." WHERE ".$this->tableletter.".`email` = :email LIMIT 0,1";
    ...
    if($this->wrongeMail + $this->dupeeMail > 0) :
      ...
      $this->query = "INSERT INTO `".$this->table_name."` " .
                                                "(" .
                    (isset($this->id_status)     ? "`id_status`, "      : "") .
                    (isset($this->name)          ? "`name`, "           : "") .
                    (isset($this->surname)       ? "`surname`, "        : "") .
                    (isset($this->username)      ? "`username`, "       : "") .
                    (isset($this->email)         ? "`email`, "          : "") .
                    (isset($this->id_hierarchy)  ? "`id_hierarchy`, "   : "") .
                    (isset($this->letter_gender) ? "`letter_gender`, "  : "") .
                                                   "`date_reg`, " .
                                                   "`date_upd`, " .
                                                   "`ip_upd`, " .
                                                   "`ref`, " .
      ...
      $this->query = $this->queryBeautifier($this->query);
                                              ...
                                              $stmt = $this->conn->prepare($this->query);
      if(isset($this->id_status))           : $stmt->bindParam(":id_status",      $this->id_status);         endif;
      if(isset($this->name))                : $stmt->bindParam(":name",           $this->name);              endif;
      if(isset($this->surname))             : $stmt->bindParam(":surname",        $this->surname);           endif;
      if(isset($this->username))            : $stmt->bindParam(":username",       $this->username);          endif;
      if(isset($this->email))               : $stmt->bindParam(":email",          $this->email);             endif;
      if(isset($this->id_hierarchy))        : $stmt->bindParam(":id_hierarchy",   $this->id_hierarchy);      endif;
      if(isset($this->letter_gender))       : $stmt->bindParam(":letter_gender",  $this->letter_gender);     endif;
